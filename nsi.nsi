...
OutFile ProyectoFinalDI-${VERSION}-win32.exe
...
Name "GRADEEXPLORER"
Caption "GRADEEXPLORER ${VERSION} para Win32 Setup"
;Icon icono.ico
...
InstallDir "$PROGRAMFILES\GRADEEXPLORER"

; check if the program has already been installed, if so, take this dir
; as install dir
InstallDirRegKey HKLM SOFTWARE\GRADEEXPLORER "Install_Dir"
...
UninstallText "Este es el desinstalador de la apliación."
...
Section "Programa"
	StrCpy $PATH "GRADEEXPLORER"
	StrCpy $PATH_ACCESO_DIRECTO "GRADEEXPLORER"

	SetOutPath $INSTDIR\$PATH

;Incluimos todos los ficheros que componen nuestra aplicación	
	File /nonfatal /a /r ".\GradeExplorer\bin\Release\"
...
;Creamos los directorios, acesos directos y claves del registro que queramos...
	...
	CreateShortCut "$SMPROGRAMS\$PATH_ACCESO_DIRECTO\GradeExplorer.lnk" \
                       "$INSTDIR\GRADEEXPLORER\GradeExplorer.exe" 
...
;Creamos también el aceso directo al instalador
	CreateShortCut "$SMPROGRAMS\$PATH_ACCESO_DIRECTO\Desinstalar.lnk" \
        ...
        WriteRegStr HKLM \
            ...
            "DisplayName" "Aplicación proyecto final ${VERSION}"
        WriteRegStr HKLM \
            SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$PATH \
...
Section "Uninstall"
	StrCpy $PATH "GRADEEXPLORER"
	StrCpy $PATH_ACCESO_DIRECTO "_GRADEEXPLORER_"
        SetShellVarContext all
